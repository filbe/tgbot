...
// load credentials from Diploi Environment variables list (Deployment -> Options -> Environment variables)
...
export const botCommand = (params: BotCommandParams) => {
  const { db, bot, commandName, argsMinLength, help, callback } = params;
  bot.command(commandName, async (ctx: BotCommandContext) => {
    try {
      const args = ctx.message.text.replace(`/${commandName}`, '').split(' ');
  ...
  return { commandName, noParams: !argsMinLength, help } as BotCommandInfo;
};

export const messageEveryone = async (
  db: sqlite3.Database,
  bot: Telegraf,
  msg: string,
  ...
  const users: {
    ...
    username: string;
    chatID: number;
  }[] = ((await rowsFromDb(db, 'User', null)) as any) || [];
